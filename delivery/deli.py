...
def delivery_deploy(request, project_id):
    ...
    project.save()
    sleep(2)
    os.system("mkdir -p /var/opt/monster/workspace/{0}/logs".format(job_name))
    os.system("mkdir -p /var/opt/monster/workspace/{0}/scripts".format(job_name))
    if app_path == "/":
...
def log2(request, project_id):
    ...
    job_name = project.job_name.name
    try:
        job_workspace = "/var/opt/monster/workspace/{0}/".format(job_name)
...
def logs_history(request, project_id):
    project = Delivery.objects.get(job_name_id=project_id)
    job_name = project.job_name.name
    log_path = "/var/opt/monster/workspace/{0}/logs".format(job_name)
